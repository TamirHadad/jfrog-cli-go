...
func UploadFile(f *os.File, url string, artifactoryDetails *auth.ArtifactoryAuthConfiguration, details *fileutils.FileDetails,
	httpClientsDetails httputils.HttpClientDetails, client *helpers.JfrogHttpClient) (*http.Response, []byte, error) {
	var err error
	if details == nil {
...
func AddAuthHeaders(headers map[string]string, artifactoryDetails *auth.ArtifactoryAuthConfiguration) map[string]string {
	if headers == nil {
		headers = make(map[string]string)
	}
	if artifactoryDetails.GetSshAuthHeaders() != nil {
		cliutils.MergeMaps(artifactoryDetails.GetSshAuthHeaders(), headers)
	}
	return headers
}
...
func filterSearchByBuild(buildIdentifier string, resultsToFilter []AqlSearchResultItem, flags CommonConf, client *helpers.JfrogHttpClient) ([]AqlSearchResultItem, error) {
	buildName, buildNumber, err := getBuildNameAndNumber(buildIdentifier, flags)
	if err != nil {
		return nil, err
	}
	query := createAqlQueryForBuild(buildName, buildNumber)
	aqlResponse, err := execAqlSearch(query, flags, client)
	if err != nil {
		return nil, err
	}
...
type CommonConf interface {
	GetArtifactoryDetails() *auth.ArtifactoryAuthConfiguration
	SetArtifactoryDetails(rt *auth.ArtifactoryAuthConfiguration)
	IsDryRun() bool
}
...
func (flags *CommonConfImpl) GetArtifactoryDetails() *auth.ArtifactoryAuthConfiguration {
	return flags.artDetails
}

func (flags *CommonConfImpl) SetArtifactoryDetails(rt *auth.ArtifactoryAuthConfiguration) {
	flags.artDetails = rt
}
